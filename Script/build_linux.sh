...
if [ $DOCKER -eq 1 ]; then
    ...
    git archive --format=tar.gz -o ChineseChess.tar.gz --prefix ChineseChess/ HEAD
    if [ -d ${BUILD_LINUX_DIR} ]; then
        rm -fr ${BUILD_LINUX_DIR}
    fi
    mkdir -p ${BUILD_LINUX_DIR}
    mv ChineseChess.tar.gz ${BUILD_LINUX_DIR}/
    chmod a+rw ${BUILD_LINUX_DIR}/ChineseChess.tar.gz
    popd
    ...
    if [ $DEB -eq 1 ]; then
        ...
        docker run --privileged ${DOCKER_PARA} \
            -e CI=${CI} \
            --volume ${REPO_ROOT}:/home/ChineseChess \
           ...
           bash -e -x -c "
           if [ ! \$CI ]; then
               tar -C ~ -xf /home/build/ChineseChess.tar.gz
               export SOURCE_CODE_DIR=~
           else
               export SOURCE_CODE_DIR=/home
           fi
           \${SOURCE_CODE_DIR}/ChineseChess/Script/build_linux.sh --deb --install=/home/install --tools=/home/tools --verbose=${BUILD_VERBOSE}
           cp \${SOURCE_CODE_DIR}/chinesechess*.deb /home/build/
           "
    fi
    ...
    if [ $APPIMAGE -eq 1 ]; then
        ...
        docker run --privileged ${DOCKER_PARA} \
            ...
            \${SOURCE_CODE_DIR}/ChineseChess/Script/build_linux.sh --appimage --install=/home/install --tools=/home/tools --verbose=${BUILD_VERBOSE}
            # Create install script
            echo \"== Create install script ......\"
            mkdir -p /home/build/install
            pushd /home/build/install
            cp \${SOURCE_CODE_DIR}/ChineseChess/ChineseChess_`uname -m`.AppImage .
            chmod a+rx ChineseChess_`uname -m`.AppImage
            cp \${SOURCE_CODE_DIR}/ChineseChess/build_appimage/AppDir/usr/share/applications/io.github.KangLin.ChineseChess.desktop .
            cp \${SOURCE_CODE_DIR}/ChineseChess/build_appimage/AppDir/usr/share/pixmaps/io.github.KangLin.ChineseChess.png .
            cp \${SOURCE_CODE_DIR}/ChineseChess/Script/install_appimage.sh install.sh
    ...
    if [ $RPM -eq 1 ]; then
        docker run --volume ${BUILD_LINUX_DIR}:/home/build \
            ...
            cp /home/build/ChineseChess.tar.gz ~/rpmbuild/SOURCES/
            tar -C ~ -xf /home/build/ChineseChess.tar.gz
            # Install getopt
            dnf install -y util-linux
            ~/ChineseChess/Script/build_linux.sh --rpm --install=/home/install --tools=/home/tools --verbose=${BUILD_VERBOSE}
            cp ~/rpmbuild/RPMS/`uname -m`/chinesechess*.rpm /home/build/
            "
    fi
    exit 0
fi
...
if [ $DEB -eq 1 ]; then
    ...
    ./build_depend.sh ${depend_para} --rabbitcommon \
        --install=${INSTALL_DIR} \
        --source=${SOURCE_DIR} \
        --tools=${TOOLS_DIR} \
        --verbose=${BUILD_VERBOSE}
...
if [ $APPIMAGE -eq 1 ]; then
    ...
    ./build_depend.sh --system_update --base --default --rabbitcommon \
        --install=${INSTALL_DIR} \
        --source=${SOURCE_DIR} \
        --tools=${TOOLS_DIR} \
        --verbose=${BUILD_VERBOSE}
...
if [ $RPM -eq 1 ]; then
    ...
    dnf builddep -y ${REPO_ROOT}/Package/rpm/chinesechess.spec
