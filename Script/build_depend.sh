...
parse_with_getopt() {
    ...
    OPTS=help,install:,source:,tools:,build:,verbose::,package:,package-tool:,system_update::,system-update::,base::,default::,macos::,qt::,rabbitcommon::
...
validate_parameters() {
    ...
    for var in SYSTEM_UPDATE BASE_LIBS DEFAULT_LIBS QT RabbitCommon ; do
        local value="${!var}"
...
if [ $BASE_LIBS -eq 1 ]; then
    ...
    if [ "$PACKAGE_TOOL" = "apt" ]; then
        ...
        # Base dependency
        package_install libssl-dev libcrypt-dev
...
if [ $QT -eq 1 ]; then
    ...
    if [ ! -d qt_`uname -m` ]; then
        ...
        if [ "`uname -m`" == "x86_64" ]; then
            aqt install-qt linux desktop ${QT_VERSION} linux_gcc_64 -m qtscxml qtmultimedia qtimageformats qt5compat
            mv ${QT_VERSION}/gcc_64 qt_`uname -m`
         elif [ "`uname -m`" == "aarch64" ]; then
            aqt install-qt linux_arm64 desktop ${QT_VERSION} linux_gcc_arm64 -m qtscxml qtmultimedia qtimageformats qt5compat
