...
if [ ! -f ~/rpmbuild/SOURCES/ChineseChess.tar.gz ]; then
    mkdir -p ~/rpmbuild/SOURCES/
    git archive --format=tar.gz --prefix=ChineseChess/ -o ~/rpmbuild/SOURCES/ChineseChess.tar.gz HEAD
fi
export RabbitCommon_ROOT=${SOURCE_DIR}/RabbitCommon
export CMAKE_PREFIX_PATH=${INSTALL_DIR}:${CMAKE_PREFIX_PATH}
export INSTALL_DIR=${INSTALL_DIR}
rpmbuild --nodebuginfo -bb Package/rpm/chinesechess.spec --define "build_time $(date '+%a %b %d %Y')"
