...
cmake "$REPO_ROOT" \
  -DCMAKE_INSTALL_PREFIX=ChineseChessControl.App/Contents \
...
if [ -d "${INSTALL_DIR}/share/qtermwidget6" ]; then
    ...
    cp -R ${INSTALL_DIR}/share/qtermwidget6 ChineseChessControl.App/Contents/Resources/
else
...
if [ -d "${MACDEPLOYQT}" ]; then
    echo "macdeployqt ......"
    ${MACDEPLOYQT} `pwd`/ChineseChessControl.App -dmg -verbose=3
else
    echo "Error: Don't found macdeployqt, please set QT_ROOT"
fi

7z a ChineseChessControl.zip ./ChineseChessControl.App

if [ -f ChineseChessControl.zip ]; then
    echo "--- Generated zip: $REPO_ROOT/ChineseChessControl.zip"
    cp ChineseChessControl.zip $REPO_ROOT/
fi
if [ -f ChineseChessControl.dmg ]; then
    echo "--- Generated dmg: $REPO_ROOT/ChineseChessControl.dmg"
    cp ChineseChessControl.dmg $REPO_ROOT/
fi
