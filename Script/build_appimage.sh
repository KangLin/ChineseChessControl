...
#    TEMP_BASE=/tmp
#fi
#BUILD_APPIMAGE_DIR=$(mktemp -d -p "$TEMP_BASE" ChineseChess-appimage-build-XXXXXX)
...
echo_status "Compile ChineseChess ......"
...
cmake --install . --config Release --strip --component QtRuntime --prefix ${INSTALL_APP_DIR}
cmake --install . --config Release --strip --component QtApplications --prefix ${INSTALL_APP_DIR}
...
export EXTRA_QT_MODULES="svg;multimedia;statemachine"
...
chmod a+x ChineseChess-`uname -m`.AppImage

cp ChineseChess-`uname -m`.AppImage $REPO_ROOT/ChineseChess_`uname -m`.AppImage
echo_status "Generated AppImage: $REPO_ROOT/ChineseChess_`uname -m`.AppImage"
